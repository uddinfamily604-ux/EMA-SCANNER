...
const TF_MAP = {"1m":{multiplier:1,timespan:"minute"},"5m":{multiplier:5,timespan:"minute"},"15m":{multiplier:15,timespan:"minute"},"30m":{multiplier:30,timespan:"minute"},"1h":{multiplier:1,timespan:"hour"},"4h":{multiplier:4,timespan:"hour"},"1D":{multiplier:1,timespan:"day"},"1W":{multiplier:1,timespan:"week"}};
...
async function fetchCandles(symbol, tf) {
  ...
  if (data.status==="ERROR"||!data.results||data.results.length===0) return null;
...
async function analyzeSymbol(symbol, ema1, ema2, ema3, tf) {
  try {
    ...
    for (let i=closes.length-slopeWindow; i<=closes.length; i++) {
      ...
      e1Arr.push(calcEMA(slice,ema1)); e2Arr.push(calcEMA(slice,ema2)); e3Arr.push(calcEMA(slice,ema3));
    }
    const e1=e1Arr[e1Arr.length-1], e2=e2Arr[e2Arr.length-1], e3=e3Arr[e3Arr.length-1];
    if (!e1||!e2||!e3) return null;
    const slope1=calcSlope(e1Arr.filter(Boolean)), slope2=calcSlope(e2Arr.filter(Boolean)), slope3=calcSlope(e3Arr.filter(Boolean));
    const spread=Math.abs(e3-e1), spreadPct=(spread/e1)*100, compressed=spreadPct<1.5;
    const allSlopingDown=slope1<0&&slope2<0&&slope3<0, allSlopingUp=slope1>0&&slope2>0&&slope3>0;
    const distFromE1=((price-e1)/e1)*100, distFromE2=((price-e2)/e2)*100, distFromE3=((price-e3)/e3)*100;
    let score=0;
    if (allSlopingDown||allSlopingUp) score+=40;
    if (compressed) score+=30;
    score+=Math.max(0,30-spreadPct*10);
    return { symbol, price:price.toFixed(2), ema1:e1.toFixed(2), ema2:e2.toFixed(2), ema3:e3.toFixed(2), spread:spread.toFixed(2), spreadPct:spreadPct.toFixed(3), slope1, slope2, slope3, allSlopingDown, allSlopingUp, compressed, distFromE1:distFromE1.toFixed(2), distFromE2:distFromE2.toFixed(2), distFromE3:distFromE3.toFixed(2), trend:allSlopingDown?"BEARISH":allSlopingUp?"BULLISH":"MIXED", score:Math.min(100,Math.round(score)) };
  } catch(e) { return null; }
}
...
function ScoreBar({score}) {
  ...
  return <div style={{display:"flex",alignItems:"center",gap:6}}><div style={{width:60,height:6,background:"#1a2332",borderRadius:3,overflow:"hidden"}}><div style={{width:`${score}%`,height:"100%",background:color,borderRadius:3}}/></div><span style={{color,fontSize:11,fontWeight:700,minWidth:28}}>{score}</span></div>;
}
...
function Stat({label,value,color="#c9d8e8"}) {
  return <div style={{display:"flex",alignItems:"center",gap:6}}><span style={{fontSize:9,color:"#2a5a7a",letterSpacing:1}}>{label}</span><span style={{fontSize:13,color,fontWeight:700}}>{value}</span></div>;
}
function DetailRow({label,val,color}) {
  return <div style={{display:"flex",justifyContent:"space-between",gap:16,marginBottom:4}}><span style={{fontSize:11,color:"#3a6e9a"}}>{label}</span><span style={{fontSize:11,color,fontWeight:700}}>{val}</span></div>;
}
...
export default function EMAScanner() {
  ...
  const runScan = useCallback(async () => {
    ...
    filtered.sort((a,b)=>sortBy==="score"?b.score-a.score:sortBy==="spread"?parseFloat(a.spreadPct)-parseFloat(b.spreadPct):a.symbol.localeCompare(b.symbol));
      ...
      <div style={{display:"flex"}}>
        <div style={{width:260,minWidth:260,background:"#0a1520",borderRight:"1px solid #1e3a5a",padding:"16px 14px",display:"flex",flexDirection:"column",gap:16}}>
          ...
          <Section title="TIMEFRAME">
            <div style={{display:"flex",flexWrap:"wrap",gap:6}}>
              {TIMEFRAMES.map(tf=><button key={tf} onClick={()=>setTimeframe(tf)} style={{padding:"5px 10px",fontSize:11,fontFamily:"inherit",background:timeframe===tf?"#0d3b5e":"#0d1b2e",border:`1px solid ${timeframe===tf?"#00b4d8":"#1e3a5a"}`,color:timeframe===tf?"#00b4d8":"#3a6e9a",borderRadius:4,cursor:"pointer",fontWeight:700}}>{tf}</button>)}
            </div>
          </Section>
          <Section title="FILTER MODE">
            ...
            {filterMode==="COMPRESSED"&&<div style={{marginTop:8}}><div style={{fontSize:10,color:"#3a6e9a",marginBottom:4}}>MAX SPREAD %</div><input type="number" step={0.1} min={0.1} max={10} value={maxSpread} onChange={e=>setMaxSpread(Number(e.target.value))} style={{width:"100%",background:"#0d1b2e",border:"1px solid #1e3a5a",borderRadius:4,color:"#00b4d8",padding:"6px 10px",fontSize:13,fontFamily:"inherit",outline:"none",boxSizing:"border-box"}}/></div>}
          </Section>
          <Section title="SORT BY">
            <div style={{display:"flex",gap:6}}>
              {["score","spread","symbol"].map(s=><button key={s} onClick={()=>setSortBy(s)} style={{flex:1,padding:"5px 4px",fontSize:10,fontFamily:"inherit",background:sortBy===s?"#0d3b5e":"#0d1b2e",border:`1px solid ${sortBy===s?"#00b4d8":"#1e3a5a"}`,color:sortBy===s?"#00b4d8":"#3a6e9a",borderRadius:4,cursor:"pointer",fontWeight:700}}>{s.toUpperCase()}</button>)}
            </div>
          </Section>
        ...
        <div style={{flex:1,overflow:"hidden",display:"flex",flexDirection:"column"}}>
          ...
          <div style={{overflowY:"auto",flex:1}}>
            {results.length===0&&!scanning&&<div style={{padding:40,textAlign:"center",color:"#2a5a7a",fontSize:13}}>{lastScan?"No results matched your filter.":"Press ▶ RUN SCAN to fetch live data"}</div>}
            {results.length>0&&(
              <table style={{width:"100%",borderCollapse:"collapse",fontSize:12}}>
                <thead><tr style={{background:"#0a1520",borderBottom:"2px solid #1e3a5a"}}>
                  {["SYMBOL","PRICE",`EMA${ema1}`,`EMA${ema2}`,`EMA${ema3}`,"SPREAD","SPREAD%","SLOPE","DIST E1","DIST E2","DIST E3","TREND","SIGNAL"].map(h=><th key={h} style={{padding:"8px 10px",textAlign:"left",color:"#2a6e9a",fontSize:10,fontWeight:700,letterSpacing:1,whiteSpace:"nowrap"}}>{h}</th>)}
                </tr></thead>
                <tbody>
                  {results.map((r,i)=>{
                    ...
                    return <tr key={r.symbol} onClick={()=>setSelectedRow(isSelected?null:i)} style={{background:rowBg,borderBottom:"1px solid #0f1e2e",cursor:"pointer"}}>
                      ...
                      <td style={{padding:"7px 10px"}}><div style={{display:"flex",gap:3}}><SlopeArrow slope={r.slope1}/><SlopeArrow slope={r.slope2}/><SlopeArrow slope={r.slope3}/></div></td>
                      ...
                      <td style={{padding:"7px 10px"}}><span style={{padding:"2px 7px",borderRadius:3,fontSize:10,fontWeight:700,letterSpacing:1,background:r.trend==="BEARISH"?"#2a0a0a":r.trend==="BULLISH"?"#0a2a0a":"#1a1a0a",color:r.trend==="BEARISH"?"#ff1744":r.trend==="BULLISH"?"#00e676":"#ffeb3b",border:`1px solid ${r.trend==="BEARISH"?"#ff1744":r.trend==="BULLISH"?"#00e676":"#ffeb3b"}`}}>{r.trend}</span></td>
          ...
          {selected&&(
            <div style={{background:"#0a1520",borderTop:"2px solid #1e3a5a",padding:"14px 20px"}}>
              ...
              <div style={{display:"flex",gap:30,flexWrap:"wrap"}}>
                <div style={{minWidth:140}}><div style={{fontSize:9,color:"#2a5a7a",letterSpacing:1,marginBottom:8,fontWeight:700}}>EMA LEVELS</div><DetailRow label={`EMA${ema1}`} val={selected.ema1} color="#00b4d8"/><DetailRow label={`EMA${ema2}`} val={selected.ema2} color="#00b4d8"/><DetailRow label={`EMA${ema3}`} val={selected.ema3} color="#00b4d8"/><DetailRow label="Price" val={selected.price} color="#e8f4ff"/></div>
                <div style={{minWidth:140}}><div style={{fontSize:9,color:"#2a5a7a",letterSpacing:1,marginBottom:8,fontWeight:700}}>EMA SPREAD</div><DetailRow label="Pts Apart" val={selected.spread} color="#ffeb3b"/><DetailRow label="% Spread" val={`${selected.spreadPct}%`} color={parseFloat(selected.spreadPct)<0.5?"#ff9800":"#ffeb3b"}/><DetailRow label="Compressed?" val={selected.compressed?"YES ●":"NO"} color={selected.compressed?"#ff9800":"#3a6e9a"}/></div>
                <div style={{minWidth:140}}><div style={{fontSize:9,color:"#2a5a7a",letterSpacing:1,marginBottom:8,fontWeight:700}}>SLOPE DIRECTION</div><div style={{display:"flex",gap:16}}>{[[`EMA${ema1}`,selected.slope1],[`EMA${ema2}`,selected.slope2],[`EMA${ema3}`,selected.slope3]].map(([l,s])=><div key={l} style={{textAlign:"center"}}><div style={{fontSize:10,color:"#3a6e9a",marginBottom:4}}>{l}</div><SlopeArrow slope={s}/><div style={{fontSize:10,color:s>0?"#00e676":"#ff1744",marginTop:2}}>{s>0?"UP":"DOWN"}</div></div>)}</div></div>
                <div style={{minWidth:160}}><div style={{fontSize:9,color:"#2a5a7a",letterSpacing:1,marginBottom:8,fontWeight:700}}>PRICE vs EMA DISTANCE</div><DetailRow label={`vs EMA${ema1}`} val={`${selected.distFromE1}%`} color={parseFloat(selected.distFromE1)>0?"#00e676":"#ff1744"}/><DetailRow label={`vs EMA${ema2}`} val={`${selected.distFromE2}%`} color={parseFloat(selected.distFromE2)>0?"#00e676":"#ff1744"}/><DetailRow label={`vs EMA${ema3}`} val={`${selected.distFromE3}%`} color={parseFloat(selected.distFromE3)>0?"#00e676":"#ff1744"}/></div>
                <div style={{minWidth:160}}><div style={{fontSize:9,color:"#2a5a7a",letterSpacing:1,marginBottom:8,fontWeight:700}}>SIGNAL STRENGTH</div><ScoreBar score={selected.score}/><div style={{marginTop:6,fontSize:10,color:"#3a6e9a"}}>{selected.score>75?"★ STRONG — All conditions aligned":selected.score>50?"◆ MODERATE — Most conditions met":"◇ WEAK — Few conditions triggered"}</div></div>
              </div>
            </div>
          )}
        </div>

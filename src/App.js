...
function calcSmoothHeikinAshi(opens, highs, lows, closes, smoothLen=10) {
  ...
  // Step 1: EMA smooth the raw OHLC
  ...
  // Step 2: Build HA candles from smoothed values
  const haO=[], haC=[];
  for(let i=0;i<n;i++){
  ...
  // Step 3: Double smooth the HA candles
  const dsmO=[], dsmC=[];
  for(let i=0;i<n;i++){
  ...
  // Current SHA direction: close > open = bullish = UP
...
async function analyzeSHAHT(symbol, tf) {
  ...
  // Algo1 = SHA direction, Algo2 = HalfTrend direction
...
function SHAHTBadge({result}){
  ...
  const agreeBg=result.agree?(result.algo1==="UP"?"#0a2a0a":"#2a0a0a"):"#1a1a0a";
  const agreeColor=result.agree?(result.algo1==="UP"?"#00e676":"#ff1744"):"#ffeb3b";
  return <div style={{display:"flex",flexDirection:"column",alignItems:"center",gap:2}}>
    ...
    {result.agree&&<span style={{padding:"1px 5px",borderRadius:3,fontSize:8,fontWeight:700,background:agreeBg,color:agreeColor,border:`1px solid ${agreeColor}`}}>✓AGREE</span>}
        ...
        {sel&&(
          <div style={{background:"#0a1520",borderTop:"2px solid #1e3a5a",padding:"10px 16px"}}>
            ...
            <div style={{display:"flex",gap:20,flexWrap:"wrap"}}>
              ...
              <div><div style={{fontSize:9,color:"#2a5a7a",marginBottom:5,fontWeight:700}}>SCORE</div>
                <ScoreBar score={sel.score}/>
                {sel.swingSignal&&<div style={{marginTop:6,padding:"5px 8px",background:"#1a0028",border:"1px solid #e040fb",borderRadius:4,fontSize:10,color:"#e040fb",fontWeight:700}}>★ SWING SELL SETUP</div>}
              </div>
            </div>
          </div>
        )}
...
function HalfTrendScanner({symbols}){
  ...
  },[symbols,tf1,_tf2,tf3,tf4,direction]);
      ...
      <div style={{width:250,minWidth:250,background:"#0a1520",borderRight:"1px solid #1e3a5a",padding:"14px 12px",display:"flex",flexDirection:"column",gap:14,overflowY:"auto"}}>
        <Section title="4 TIMEFRAMES (type any)">
          <div style={{display:"flex",gap:8,marginBottom:8}}>
            ...
            <TFInput label="TF2 CONFIRM" value={_tf2} onChange={_setTf2}/>
          </div>
          <div style={{display:"flex",gap:8}}>
            <TFInput label="TF3 ENTRY" value={tf3} onChange={setTf3}/>
            <TFInput label="TF4 FINE" value={tf4} onChange={setTf4}/>
          </div>
          <div style={{fontSize:9,color:"#2a4a6a",marginTop:8}}>30s · 1m · 2m · 5m · 15m · 30m · 1h · 4h · 1D</div>
          <div style={{marginTop:8,padding:"8px",background:"#0d1b2e",borderRadius:4,border:"1px solid #1e3a5a"}}>
            ...
            {[["SWING","1h","15m","5m","1m"],["SCALP","5m","2m","30s","15s"],["DAY","4h","1h","15m","5m"]].map(([label,a,b,c,d])=>(
              <button key={label} onClick={()=>{setTf1(a);_setTf2(b);setTf3(c);setTf4(d);}}
      ...
      <div style={{flex:1,overflow:"hidden",display:"flex",flexDirection:"column"}}>
        ...
        <div style={{overflowY:"auto",flex:1}}>
          ...
          {results.length>0&&(
            <table style={{width:"100%",borderCollapse:"collapse",fontSize:11}}>
              <thead><tr style={{background:"#0a1520",borderBottom:"2px solid #1e3a5a"}}>
                {["SYMBOL","PRICE",`${tf1.toUpperCase()} BIAS`,`${_tf2.toUpperCase()} CONFIRM`,`${tf3.toUpperCase()} ENTRY`,`${tf4.toUpperCase()} FINE`,"MATCH","SIGNAL"].map(h=>
      ...
      <div style={{width:250,minWidth:250,background:"#0a1520",borderRight:"1px solid #1e3a5a",padding:"14px 12px",display:"flex",flexDirection:"column",gap:14,overflowY:"auto"}}>
        ...
        <div style={{padding:"10px",background:"#0d1b2e",border:"1px solid #1e3a5a",borderRadius:6}}>
          ...
          <div style={{fontSize:10,color:"#3a6e9a",lineHeight:1.8}}>
            ...
            <span style={{color:"#e040fb",fontWeight:700}}>★ SIGNAL</span> = BOTH agree on ALL 4 timeframes<br/><br/>
            This is your $300 indicator! 🔥
          </div>
        </div>
      ...
      <div style={{flex:1,overflow:"hidden",display:"flex",flexDirection:"column"}}>
        ...
        <div style={{overflowY:"auto",flex:1}}>
          {results.length===0&&!scanning&&<div style={{padding:40,textAlign:"center",color:"#2a5a7a",fontSize:13}}>
            Press ▶ SHA+HT SCAN to find signals
          </div>}
          {results.length>0&&(
            <table style={{width:"100%",borderCollapse:"collapse",fontSize:11}}>
              <thead><tr style={{background:"#0a1520",borderBottom:"2px solid #1e3a5a"}}>
                {["SYMBOL","PRICE",`${tf1.toUpperCase()}`,`${tf2.toUpperCase()}`,`${tf3.toUpperCase()}`,`${tf4.toUpperCase()}`,"MATCH","SIGNAL"].map(h=>
                  <th key={h} style={{padding:"7px 8px",textAlign:"left",color:"#2a6e9a",fontSize:9,fontWeight:700,letterSpacing:1,whiteSpace:"nowrap"}}>{h}</th>)}
              </tr></thead>
              <tbody>
                {results.map((r,i)=>{
...
export default function App(){
  ...
  const tabs=[
    ...
    {id:"ht",label:"★ HALFTREND MTF",color:"#e040fb"},
    {id:"shaht",label:"🔥 SHA + HALFTREND",color:"#ff9800"},
  ];
  ...
  return(
    <div style={{fontFamily:"'Courier New',monospace",background:"#080e1a",minHeight:"100vh",color:"#c9d8e8",display:"flex",flexDirection:"column"}}>
      {/* HEADER */}
      <div style={{background:"linear-gradient(90deg,#0d1b2e,#0a1520)",borderBottom:"1px solid #1e3a5a",padding:"10px 20px",display:"flex",alignItems:"center",justifyContent:"space-between",flexWrap:"wrap",gap:10}}>
        <div style={{display:"flex",alignItems:"center",gap:12}}>
          ...
          <span style={{fontSize:10,color:"#00b4d8",padding:"2px 8px",border:"1px solid #1e3a5a",borderRadius:3}}>v5.0</span>
        </div>
      ...
      <div style={{flex:1,display:"flex",overflow:"hidden"}}>
        ...
        {activeTab==="shaht"&&<SHAHTScanner symbols={symbols}/>}
      </div>
